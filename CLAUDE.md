...
- `internal/config/marshal.go` imports the four restore adapter packages for `RestoreArgs`/`OplogReplayArgs` type names referenced in `Build*RestoreArgs`/`BuildMongoOplogReplayArgs` return types (spec 036 clarification Q1). The dump-side equivalent was **removed by spec 040 / PRD 27** — dump args now build via `ports.DumpArgsFactory` + `internal/adapters/dump/registry.go::NewArgsFactory` (config translates its YAML `BackupJob` into the pure `ports.DumpJobSpec`). A future spec (PRD 28) may introduce `ports.RestoreArgsFactory` to remove the remaining restore direction.
...
`specs/040-dump-args-factory/plan.md`
<!-- SPECKIT END -->
