...
- Adapter dumps (`internal/adapters/dump/{pg,mysql,mariadb,mongo}/`) import `internal/adapters/db_probe` for `CheckConnectivity` / `CheckMongoConnectivity` (spec 037; replaces prior `internal/backup/sql` cross-import). Documented; no port until adapter dumps accept `ports.DBProber` via constructor injection (PRD 30 candidate). **This is the last remaining adapter→adapter exception** — the Mongo PEM cross-axis import was removed by spec 042 / PRD 29 (lifecycle relocated to the neutral `internal/adapters/mongo_tls/`).
...
`specs/042-mongo-tls-neutral-package/plan.md`
<!-- SPECKIT END -->
