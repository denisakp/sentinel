...
The signature check above proves **who published** the release. This check
proves something different and complementary: **how the binary was built** —
from which exact source commit, by which workflow, without manual tampering
in between. Each recent release ships a
[SLSA](https://slsa.dev) build-provenance attestation
(`multiple.intoto.jsonl`), generated by a process with its own identity,
independent of the job that built the binaries — so a compromise of the
build job alone could not also forge this attestation.
...
A tampered archive, or an attestation from an unexpected source repository or
tag, is rejected. This check and the signature check above are independent —
use either or both; neither replaces the other.

> **Note:** releases published before this feature ship no provenance
> attestation.
