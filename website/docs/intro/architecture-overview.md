...
**Credentials never appear in process arguments.** Configuration fields ending in `_env` name an
...
Sentinel has no `--password` flag; it has `--password-env` and `--password-file`.
