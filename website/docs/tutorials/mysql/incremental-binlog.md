---
title: Binary-log archival on MySQL
description: Enable incremental backup on MySQL, archive binary logs beside each artifact, and see why the archived logs cannot be replayed by a restore job.
sidebar_position: 4
---

By the end of this page you will have a MySQL backup job that builds an incremental chain and packs
...
You will also know precisely where incremental support stops in v1.4.0, which on MySQL matters more
...
If you started the container on the [first page](./first-backup.md) you already have the naming.
Recreate it with a bind mount so the logs are reachable:
...
The `chmod 0777` is there because the MySQL server process inside the container writes as its own
...
```bash
mysql -h 127.0.0.1 -P 3307 -u root -e "SHOW VARIABLES LIKE 'log_bin%';"
ls binlogs/
```

**What to look for**: `log_bin` reported as `ON`, `log_bin_basename` ending in `mysql-bin`, and at
least one file named `mysql-bin.000001` in `binlogs/` alongside a `mysql-bin.index`. If `binlogs/` is
empty, the bind mount or the `--log-bin` path did not take effect, and everything below will fail
with `no_binlog_files_found`.
...
- **`mysql.binlog_path`** points at the directory to archive. A relative path is resolved against the
  working directory. This key is required whenever incremental backup is enabled on a MySQL or
  MariaDB job.
...
:::caution `binlog_check` can never fire
The prerequisite function that would report `log_bin_off` is called with its "binary logging is
enabled" argument hard-coded to `true`, so the only thing actually checked is `binlog_path`. A server
with binary logging switched off passes configuration validation and fails later, when the archival
step finds no files to pack. Treat Step 1's `SHOW VARIABLES` check as the real check.
:::

The `mysql:` block keeps its name on a MariaDB job too. There is no `mariadb:` key.

## Step 3: Watch the validator enforce `binlog_path`

These three are worth provoking once so you recognise them later. All were captured by running them.
...
```text
Error: invalid config "sentinel.yaml": backup 'shop': mysql.binlog_path is required for incremental backup (type=mysql)
```

Point it at a directory that does not exist and validate again:

```text
Error: invalid config "sentinel.yaml": backup 'shop': mysql.binlog_path must exist and be locally mounted (type=mysql)
```
...
This is the part that is genuinely MySQL-specific. Look in the backup directory:
...
**What to look for**: one entry per `mysql-bin.NNNNNN` file, stored flat with no directory prefix,
in sorted file-name order. The `mysql-bin.index` file is deliberately excluded.
...
Three fields there are populated only for MySQL and MariaDB:
...
artifact until you expire them server-side.
:::
...
The replay machinery exists: Sentinel can pipe `mysqlbinlog --stop-datetime=…` or
`--stop-position=…` into the `mysql` client, and it knows how to unpack a `.binlogs.tar` first. It is
wired to run after an incremental restore has applied its base artifact, and only then. Because no
incremental restore ever gets past planning on MySQL, that code is unreachable from a configured job.

To recover data on MySQL today, restore an artifact with a plain `full` job as on the
[restore page](./restore.md), and if you need the binary logs applied on top, run `mysqlbinlog`
yourself against the archive Sentinel produced. The archive is a plain tar of standard binary-log
files, so nothing about it is Sentinel-specific.
...
restore is planned, MySQL and MariaDB use binary logs, and MongoDB uses the oplog and requires a
replica set. See [Incremental and PITR](../../concepts/incremental-pitr.md).
:::

## What just happened

Enabling `incremental_backup` changed what Sentinel records around each dump and added one step
after it; it did not change how the dump is produced. Before each run Sentinel reads the job's recent
...
reads that manifest back, and, on MySQL, declines to act on it.
