---
title: Restoring a MySQL backup
description: Rehearse a restore with a dry run, then apply the artifact into a second MySQL database and confirm the rows came back.
sidebar_position: 3
---
...
- The working directory, configuration, and `backups/shop.sql` artifact from
  [Your first MySQL backup](./first-backup.md). This page edits that same `sentinel.yaml`.
- The `sentinel-mysql-tutorial` container still running. If you stopped it, restart the track from
  the previous page; a restore needs both a live server and an artifact.
- `mysql` on your `PATH`. Sentinel applies a MySQL restore by piping the artifact into the `mysql`
  client, not with a dedicated restore binary.
...
brings up a `mysql:lts` instance on host port `3307` alongside the other supported engines.
...
- **`conflict_strategy: error`** is the default and the safe one: fail rather than overwrite. See the
  note below for what the alternatives actually do on MySQL.
...
:::caution `replace` and `ignore` are the same thing on MySQL
Of the three strategies the validator accepts, only `error` changes behaviour. Both `replace` and
`ignore` are translated into a single `--force` argument to the `mysql` client, which tells it to
...
not stop". `allow_cascade`, the PostgreSQL companion to `replace`, is rejected on a MySQL job.
:::
...
of engine. Those are `pg_restore` options; the `mysql` client rejects them and the restore fails
before it applies anything. Leave `restore_options` unset on MySQL.
:::
...
```bash
mysql -h 127.0.0.1 -P 3307 -u root -e "CREATE DATABASE shop_drill;"
```
...
Note that Sentinel does not create the target for you on MySQL. The `mysql` client is invoked with
the database name as its final argument, so the database must already exist. Its pre-flight
connectivity probe will not catch a missing one: the probe runs `SELECT 1` with no database
selected, so it passes, and the failure arrives later when the dump is applied.
...
neither of them completes on MySQL.
...
the `mysql` client stop at the first error rather than plough on, but it is a guard rail, not a
...
```bash
mysql -h 127.0.0.1 -P 3307 -u root shop_drill -e \
...
**`connectivity check failed - cannot connect to MySQL database root@127.0.0.1:3307`**: the restore
adapter runs `mysql … -e "SELECT 1"` against the server before applying anything. This is what you
get when the container is stopped, when the port is wrong, or when the password is wrong. Note that
the password reaching this check comes from `password_env` only; a `defaults_file` on the backup job
does not apply to restore jobs.

**`failed to start mysql command`**: the `mysql` client exited non-zero while applying the dump. With
`conflict_strategy: error` the likeliest cause is that the target already contains the objects in the
artifact, which is what you get from running Step 4 twice. Drop and recreate the target:
...
with a `mysql` query, as Step 5 does.

## What just happened

Sentinel resolved the object in `./backups`, staged it under `./staging`, verified its SHA-256
against the manifest, ran `mysql` with the staged file on standard input, removed the staged copy,
