...
func TestPgDumpArgsBuilder(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args:    &PgDumpArgs{Username: "test", Storage: &storage.Params{OutName: "test.sql"}, PgOutFormat: "p", Compress: false},
			want:    nil,
			wantErr: true,
		},
		{
			name:    "Username missing - error expected",
			args:    &PgDumpArgs{Host: "localhost", Port: "5432", Database: "test", Storage: &storage.Params{OutName: "test.sql"}, PgOutFormat: "p", Compress: false},
			want:    nil,
			wantErr: true,
		},
		{
		...
		},
		{
			...
			args:    &PgDumpArgs{Username: "test", Database: "test", Storage: &storage.Params{OutName: "test.backup"}, PgOutFormat: "c", Compress: true, CompressionAlgorithm: "invalid"},
			want:    nil,
			wantErr: true,
		},
		{
			name:    "invalid compression level - error expected",
			args:    &PgDumpArgs{Username: "user", Database: "test", Storage: &storage.Params{OutName: "test.backup"}, PgOutFormat: "c", Compress: true, CompressionAlgorithm: "gzip", CompressionLevel: 10},
			want:    nil,
			wantErr: true,
		},
		{
