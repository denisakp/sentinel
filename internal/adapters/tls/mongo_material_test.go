...
func TestPrepareMongoTLS_SeparateCertKey_Plain(t *testing.T) {
	...
	material, args, err := internaltls.PrepareMongoTLS(cfg, "job-1")
	if err != nil {
...
func TestPrepareMongoTLS_CombinedPEM_NoTempFile(t *testing.T) {
	...
	pre := snapshotMaterialFiles(t)
	material, args, err := internaltls.PrepareMongoTLS(cfg, "job-combined")
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}
...
func TestPrepareMongoTLS_EncryptedKey_PKCS8(t *testing.T) {
	...
	material, _, err := internaltls.PrepareMongoTLS(cfg, "job-enc")
	if err != nil {
		t.Fatalf("prepare: %v", err)
	}
	defer material.Close()
...
func TestPrepareMongoTLS_ErrorCases(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			pre := snapshotMaterialFiles(t)
			cfg := tt.cfgFn(&p)
			material, _, err := internaltls.PrepareMongoTLS(cfg, "job-err")
			if material != nil {
				defer material.Close()
			}
			if err == nil {
